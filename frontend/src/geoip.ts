...
const downloadArrayBuffer = async (url: string, retry = 3): Promise<ArrayBuffer | null> => {
  try {
...
export async function lookupIp(ipString: string): Promise<IpLookupResult> {
  ...
  const endIp = isv4
    ? new Uint32Array(dataBuffer.slice((recordCount + line) * ipSize, (recordCount + line + 1) * ipSize))[0]
