...
function RepeatElements({ children, total }: { children: JSX.Element[] | JSX.Element; total: number }) {
...
export const App = () => {
  ...
  const fetchWandbData = useCallback(() => {
    getData("nous_research", "distro-live-test", "15b-100bt-17", 5000).then((data) => {
      if (data) {
        setWandbRun(data);
      } else {
        setError(true);
  ...
  useEffect(() => {
    if (Object.keys(wandbRun?.summary ?? {}).length > 0 && fading === "loading") {
      setFading("fading");
      setTimeout(() => {
        setFading(false);
      }, 2000);
  ...
  return (
    <>
      {fading && <LoadingScreen fading={fading} warmupRun={wandbRun} error={error} />}
...
const Run: React.FC<{
  ...
  const numTotalTokens =
    run.config.total_steps * run.config.data_indicies_per_batch * run.config.batches_per_round * tokensPerBatch;

  const numCompletedTokens =
    run.summary._step * run.config.data_indicies_per_batch * run.config.batches_per_round * tokensPerBatch;

  const evals = (Object.keys(run.summary.eval) as Array<keyof typeof run.summary.eval>).map(
    (evalName, index) =>
      ({
        points: run.history
    ...
    <div className="font-black h-screen w-screen p-4 text-primary flex flex-col animate-fadeIn">
      <div className="w-full flex md:flex-row flex-col items-center place-content-between font-eva">
        <div className="w-full max-w-[30em] flex items-center">
          ...
          <TextStretcher className="font-normal text-plain flex-grow h-32 pb-2 px-4">NOUS DisTrO</TextStretcher>
  ...
  return (
    ...
    >
      ...
      <div className="text-9xl font-eva text w-[40vw] p-[1vw]">
        <TextStretcher className="w-[10vw] h-[5vw] mt-[1vw]">NOUS</TextStretcher>
        ...
        {error ? (
          <>
            <TextStretcher className="text-bad w-full h-[15vw]">Failure to load</TextStretcher>

            <TextStretcher className="w-full h-[5vw] pt-[2vw]">TRY LATER</TextStretcher>
          </>
        ) : (
          <TextStretcher className="w-full h-[15vw]">INITIALIZING...</TextStretcher>
        )}
        {warmupRun && (
          <>
            <TextStretcher className="w-full h-[3vw] mt-[2vw]">{`RUN ${warmupRun.displayName}`}</TextStretcher>
            <TextStretcher className="w-full h-[2vw] mt-[2vw]">PLEASE WAIT</TextStretcher>
          </>
        )}
      </div>
      <div className="pt-4">
...
function TrainingProgress({
  ...
  const estTimeRemaining = (numTotalTokens - numCompletedTokens) / tokensPerSecond;
    ...
    <div className="flex flex-col py-4">
      ...
      <div className="flex flex-row">
        ...
        <div className="flex flex-col text-center pl-4 text-xl w-40">
          ...
          <div className="font-eva">{formatNumber(tokensPerSecond, 0)}tok/s</div>
...
function RunMembers({ nodes }: { nodes: Record<string, { bandwidth: number }> }) {
  const nodeTotalBandwidth = Object.values(nodes).reduce((a, b) => a + b.bandwidth, 0);
  ...
  return (
    <div className="mx-8 h-full overflow-hidden">
      <div className="w-full border-2 border-primary mb-2 text-center bg-primary text-backdrop p-2 font-eva">
        <TextStretcher className="pb-2">PER-NODE NETWORK TRANSFER RATE</TextStretcher>
