...
const PlusSymbol: React.FC<PlusSymbolProps> = ({ x, y, size = 5, thickness = 2, className = "stroke-grid" }) => {
...
> = ({ half, x, y, size, thickness, centerLineThickness, className = "stroke-grid" }) => {
  ...
  return (
    <g>
      {half !== "right" && (
        <>
          <line {...verticalLineLeft} className={className} strokeWidth={thickness} />
          <line {...horizontalLineLeft} className={className} strokeWidth={thickness} />{" "}
        </>
      )}

      {half !== "left" && (
        <>
          <line {...verticalLineRight} className={className} strokeWidth={thickness} />
          <line {...horizontalLineRight} className={className} strokeWidth={thickness} />
        </>
      )}
    </g>
  );
...
export const GridOfPlusSymbols: React.FC<{
  ...
  thickness: number;
  centerLineThickness: number;
}> = ({ verticalLinePositions, horizontalLinePositions, xScale, yScale, size, thickness, centerLineThickness }) => {
  return (
    <>
      {verticalLinePositions.slice(0, -1).map((x, i) => {
        ...
        return horizontalLinePositions.map((yPos, rowIndex) => (
          <PlusSymbol key={`full-plus-${i}-${rowIndex}`} x={midX} y={yScale(yPos)} size={size} thickness={thickness} />
        ));
      })}
      ...
      {verticalLinePositions.map((x, i) => {
        return horizontalLinePositions.map((yPos, rowIndex) => (
          <SplitPlusSymbol
            ...
            half={i === 0 ? "right" : i === verticalLinePositions.length - 1 ? "left" : "full"}
          />
        ));
      })}
    </>
