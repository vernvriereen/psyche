...
> = ({
	...
	for (
		let i = xDomain[0];
		i <= xDomain[1];
		i += xMarkersEvery / majorTicksPerVerticalLine
	) {
	...
	for (
		let i = xDomain[0];
		i <= xDomain[1];
		i += xMarkersEvery / (majorTicksPerVerticalLine * minorTicksPerMajorTick)
	) {
	...
	const ordinalScale = scaleOrdinal({
		domain: lines.map((line) => {
			return `${line.label}: ${(line.renderValue ?? ((x) => x.toFixed(3)))(line.points.at(-1)?.y ?? 0)}${line.unit ?? ""}`;
		}),
	...
	const handleTooltip = useCallback(
		(
			...
			if (closestPoint) {
				...
				setTooltipLeft(
					xScale(closestPoint.point.x) + margin.left + padding.left,
				);
		...
		<div className="relative w-full h-full">
					...
					<Axis
						...
						tickComponent={({ formattedValue, ...tickProps }) =>
							formattedValue && (
								...
								>
									<tspan dx={formattedValue === "0" ? "0" : "-0.5ch"}>
			...
			{tooltipData && (
				...
				>
					...
					<div>
						{tooltipData.x}:{" "}
						{(tooltipData.line.renderValue ?? ((x) => x.toFixed(2)))(
							tooltipData.y,
						)}
				...
				{title && (
					<>
						<div className="xl:w-32 md:w-24 w-16">
							...
							{singleLine && (
								<TextStretcher>{`@${(singleLine.renderValue ?? ((x) => x.toFixed(2)))(singleLine.points[singleLine.points.length - 1].y)}${lines[0].unit ?? ""}`}</TextStretcher>
							)}
						</div>
