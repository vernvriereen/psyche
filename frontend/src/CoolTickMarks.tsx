...
export const CoolTickMarksInner: React.FC<BottomAxisProps> = ({
	width,
	height,
}) => {
	...
	return (
		<svg width={width} height={height} role="img" aria-label="plus-shaped grid">
			<Line
				from={{ x: 0, y: barWidth / 2 }}
				to={{ x: width, y: barWidth / 2 }}
				strokeWidth={4}
				stroke="currentColor"
			/>
			<Line
...
export const CoolTickMarks: React.FC<{ className?: string }> = (props) => {
	return (
		<ParentSize className={`w-full h-full overflow-hidden ${props.className}`}>
			{({ width, height }) => (
				<CoolTickMarksInner width={width} height={height} {...props} />
			)}
		</ParentSize>
	);
};
