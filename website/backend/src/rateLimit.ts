...
export function makeRetryPromise<T extends (...args: any[]) => Promise<any>>(
  ...
  const {
    ...
    retryMult = 2.5,
    onRetry = (attempt, delay) => console.warn(`retrying attempt ${attempt} after ${delay}ms...`)
  } = options;
...
export function makeRateLimitedFetch(): typeof fetch {
  ...
  return makeRetryPromise(
    ...
    },
    {
      ...
      onRetry: (attempt, delay) => console.warn(`fetch failed on attempt ${attempt}. retrying after ${delay}...`)
    }
  );
}